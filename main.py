...
from models.models import StackedTransformersClasif, BertCRF
...
#            
#            embed_large = BertEmbedding(data.get_vocab('words'), model_dir_or_name='google/bigbird-roberta-large',
#                        pool_method='last', requires_grad=True, layers='0,-1,-2,-3,-4,-5', 
#                        include_cls_sep=False, dropout=0.2, auto_truncate=True,
#                      word_dropout=0.01)
#            embed_base = BertEmbedding(data.get_vocab('words'), model_dir_or_name='google/bigbird-roberta-large', #dslim/bert-large-NER',
#                        pool_method='last', requires_grad=True, layers='0,-1,-2,-3,-4,-5', 
#                        include_cls_sep=False, dropout=0.2, auto_truncate=True,
#                      word_dropout=0.01)
#            
...
#            embed = StackEmbedding([embed, embed_large], dropout=0.0, word_dropout=0.02)
...
#                    f.write(str(word) + '\t' + str(label) + '\t' + str(label1) + '\tO\tO\tO\t_\t_\t_\t_\n')
                for word, label in zip(i['raw_words'], labels):
                    f.write(str(word) + '\t' + str(label)+ '\tO\tO\tO\t_\t_\t_\t_\n')
            f.write('\n')
    return predictions
...
def main():
    ...
    if args.model == 'bert':
        ...
        model = StackedTransformersClasif(tag_vocabs=[data_bundle.get_vocab('target')],
