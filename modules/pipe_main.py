...
import pandas as pd
def _read_text(path, encoding='utf-8',sep=None, indexes=None, dropna=True):
...
#        print(sample)
        try:
            zip_sample = list(map(list, zip(*sample)))
            zip_sample = [zip_sample[i] for i in indexes]
        except:
...
#            self.indexes = indexes
    ...
    def _load(self, path):
        ds = DataSet()
        idx = 0
#        for element in _read_text(path, indexes=self.indexes, dropna=self.dropna):
        for idx, data in _read_text(path).iterrows():
...
#            import pdb;pdb.set_trace()
            ...
            data = [str(data['text_normal']), str(data['string_label'])]
...
#                self.convert_tag, field_name=Const.TARGET, new_field_name=Const.TARGET)
        ...
        #import pdb;pdb.set_trace()
        _indexize(data_bundle, input_field_names=[Const.INPUT], target_field_names=['target'], vocabulary=self.vocabulary)
